...
import { Palette, Paintbrush, User, Lock, ChevronLeft } from 'lucide-react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const SettingsListItem = ({ label, value, onClick }: { label: string, value?: string, onClick: () => void }) => (
    <button onClick={onClick} className="flex items-center justify-between w-full text-right p-4 border-b transition-colors">
        <div>
...
export default function StudentSettingsPage() {
    const [view, setView] = useState('main'); // 'main', 'profile', 'appearance', 'account'
    ...
    const renderMainView = () => (
         <Card>
            <CardHeader>
                ...
                <CardDescription>إدارة ملفك الشخصي وإعدادات حسابك.</CardDescription>
            </CardHeader>
            <CardContent className="p-0">
                <div className="divide-y">
                     <SettingsListItem label="الملف الشخصي" value="عرض وتعديل معلوماتك" onClick={() => setView('profile')} />
                     <SettingsListItem label="المظهر" value="تخصيص ألوان التطبيق" onClick={() => setView('appearance')} />
                     <SettingsListItem label="الحساب" value="إدارة أمان حسابك" onClick={() => setView('account')} />
                </div>
            </CardContent>
        </Card>
    );
    ...
    const renderProfileView = () => (
        <Card>
            {renderSubViewHeader("الملف الشخصي", () => setView('main'))}
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground">
                                تحديث اسمك، بريدك الإلكتروني، وصورتك الشخصية.
                            </p>
                        </div>
                    </div>
                    <Button asChild variant="outline" className="w-full sm:w-auto flex-shrink-0">
                        <Link href="/student/profile">الانتقال للملف الشخصي</Link>
                    </Button>
                </div>
            </CardContent>
        </Card>
    ...
    const renderAppearanceView = () => (
         <Card>
            {renderSubViewHeader("المظهر", () => setView('main'))}
            <CardContent className="pt-6">
                 <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between rounded-lg border p-4 gap-4">
                    <div className="flex items-center gap-3">
    ...
    const renderAccountView = () => (
        <Card>
            {renderSubViewHeader("الحساب", () => setView('main'))}
